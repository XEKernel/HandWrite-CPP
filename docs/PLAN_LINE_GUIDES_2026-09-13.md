...
| **2. UI 与手绘** | 抽取 `ImageCanvasWidget`、`LineGuideDialog`、拖动采点 + 平滑 + 关键曲线插值、基线滑块 | 用户可自助标定整页 | 2.8.0 | ✅ **已完成（2026-09-25）** |
| **3. 水平透视补偿** | 锚点四角插值出 `sx(x)`，逐字 `painter.scale` | 斜拍照片也自然 | 2.9.0 | ✅ **已完成（2026-09-25）** |
| **4. 自动检测（可选）** | 附录 A 的算法 + 候选线确认 | 「自动检测」按钮 | 2.9.0 | ✅ **已完成（2026-09-25）** |
| **5. 扩展** | 作文格/田字格 2D 导引、竖排竖线导引 | — | 之后 | 待开工 |

### 批次 4 实施记录（2026-09-25）

附录 A 的算法在实现时做了两处重要修正（附录 A 的设计有缺陷）：

1. **「逐行暗像素占比」→「中间亮度像素占比」**。
   附录 A 原设计找"最暗行"，但照片里的深色桌面/背景比横线**更暗**，
   "找最暗"会被背景彻底带偏（真实复现：梯形斜拍样例图上中间偏差达到线距的 158%）。
   改为三分类亮度（纸亮 / 横线中 / 背景暗，阈值由直方图最高峰自适应），
   横线行的「中间亮度像素占比」最高，纸面与背景行都接近 0，天然免疫背景干扰。
2. **"等距规则化"的对象是直线位置，弯曲由后续的逐列跟踪补足**。
   逐列跟踪以上一个采样点为搜索中心（而不是固定的直线位置）——
   搜索窗口只需覆盖相邻列之间的弯曲变化量，整条线弯几十像素也能跟过去。

实施中另踩三个坑（都已修复并有注释）：

- **平滑核**：印刷横线常只有 1px 粗，等权平均 `[1,1,1]/3` 一次就把峰值从 1.0 压到 0.33，
  掉到检测阈值以下。改用 `[1,2,1]/4` 加权核 + 阈值 0.35。
- **偶数条数的中间偏差**：条数为偶数时"正中间"没有线，固定用 0.5 权重评估中间曲线
  会天然差出半个线距，让直横线也被误判成「弯曲非线性」。权重必须按 midIdx/(n-1) 取。
- **背景列块**：横线只覆盖纸面宽度，首列块（x < 纸左边界）全是背景时窗口内 ratio 全 0，
  若取窗口边界会把曲线拽到相邻横线上，整条线偏移一个线距。窗口内找不到横线时必须**保持当前 y**。

验证：`tools/make_sample_notebook.py` 生成 skew/flat 两种样例图，
检测均得 22 条、关键曲线 2 条、中间偏差 dev=0.015/0.004（此前 skew 是 1.58）；
单测新增直横线/弯曲/空白/空图/小图/回填插值 6 组断言，`ctest` **170/170**，零告警。
检测 → 写出预设片段 → CLI 渲染的完整链路人工确认通过。

**未完成**：批次 5（作文格/竖排扩展）。

### 批次 4 补充完善（2026-09-26）

隔天回看时又发现 4 处缺陷，已全部修复（详见 CHANGELOG 的 2.9.0「修复」小节）：

| # | 问题 | 影响面 | 根因 |
|---|------|--------|------|
| 1 | `build()` 不保证关键曲线有序 | **CLI 路径全错** | 排序只做在 UI 层；预设文件里的顺序不可控 |
| 2 | 拖拽曲线时会拖到别的曲线上 | GUI 交互 | 拖拽每帧重排序，而下标索引失效 |
| 3 | 检测曲线两端带纸外垃圾段 | 检测精度 | 无横线列块保留等距初值，初值有累积误差 |
| 4 | 只画 1 条曲线时确定会静默失效 | 易用性 | 插值模式判无效但无反馈 |

另有两处体验改进：Ctrl+Z 撤销、检测期间等待光标。

验证手段（本轮新增，可复用）：

- **顺序无关性**：同一组关键曲线正序/逆序各写一份预设，渲染两张图比 MD5 —— 修复前不同、修复后一致。
- **轻量检测验证程序**：只链 `core/config/tools` + Qt Core/Gui，打印检测到的曲线数量、
  间距、中间偏差、以及首末曲线的 x/y 范围与点数，用来确认"纸外垃圾段已裁掉"。
  比拉起完整 GUI 快得多。

### 仍未做（评估后暂缓）

- **画布滚轮缩放**：高分辨率照片在对话框里缩小显示后手绘困难，目前靠 3× 放大镜缓解。
  加缩放要改 `ImageCanvasDialog` 基类的坐标换算（`CalibrationDialog` 共用），有回归风险，暂缓。
- **检测 ROI 框选**：照片里有多个本子/其他物件时无法限定范围。目前靠"裁掉纸外垃圾段"部分缓解。
- **`followCurve = false` 的语义**：实现为「y 仍跟随曲线、但字符不旋转」（保持字形不扭曲），
  与初版规划写的「退化为直线 + 行首倾角」不同 —— 当前行为更实用，以此为准。

### 批次 2 实施记录（2026-09-25）

- 抽取 `ImageCanvasDialog` 基类（载入背景 / 等比缩放居中 / 坐标换算 / resize 重算），
  `CalibrationDialog` 改为继承它 —— 已用 offscreen 渲染截图确认**锚点对话框无回归**
- `LineGuideDialog` 交互：按住鼠标沿横线拖动描线 → 采点按 ≥3px 抽稀 → 松手 `normalize(2)` 平滑；
  关键曲线**按垂直位置自动排序**（插值必须在空间相邻的两条之间做，用户描线先后顺序不能决定分段）；
  端点拖拽微调 / 整条整体平移 / `Delete` 删除 / `Esc` 取消 / 撤销栈；
  描线时显示水平参考线 + **3× 放大镜**（弯曲的线在缩略图上看不真切，这条是画准的关键）
- 参数面板：条数、是否插值、基线位置滑块 + px 微调、逐字跟随弯曲、每行占几条线
- 主窗口「背景图片」分组新增「横线...」；锚点未标定时提示但**不强制**
- 验证：零告警、`ctest` 152/152、offscreen 截图确认对话框与主窗口布局正常

### 批次 3 实施记录（2026-09-25）

- `PageRenderData` 新增 `lineScaleX`（每行水平缩放）与 `lineOriginX`（每行文字块左边界）
- `layoutPages` 用锚点四角双线性插值算出每行文字块在照片上的实际左边界与宽度
  （归一化 u 取 leftMargin/rightMargin 对应的位置，v 取行基线对应的位置）
- `renderPageStatic` 逐行施加变换 `x → originX + (x - contentLeft) * scale`，
  即先 translate 再 scale，使逻辑坐标 `contentLeft` 正好落在 `originX` 上
- ⚠️ 第一版只做了 `scale` 而把原点固定在 `contentLeft`，结果行首仍停在页面标称位置、
  文字画到纸外面去了 —— **平移与缩放必须一起做**，这是本批最容易错的地方
- 缩放限幅 `[0.35, 2.8]`，避免锚点标歪时把字拉成离谱宽度
- 验证：梯形斜拍照片（上窄下宽）下上部字明显变窄、下部接近正常，文字左边界贴合纸的左斜边；
  **正拍对照**（锚点＝画布四角）缩放恒为 1.0，结果与补偿前完全一致
- 本批顺带修掉一个既有 bug：**CLI 加载预设时背景锚点校准永远不生效**
  （`cal.enabled = cal.isValid()` 顺序反了 —— `isValid()` 自身要求 `enabled` 为真）
