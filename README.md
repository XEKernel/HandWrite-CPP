...
- **横线导引** — 沿作业本上的印刷横线描线，文字逐行排布在横线内。
  ...
  也可用 **「自动检测」** 一键标定（三分类亮度 + 等距规则化 + 逐列跟踪，检测后可继续微调）。
  建议先用「锚点」标定四角再描线 —— 锚点管页面四角几何，横线管每行的弯曲，两者配合最好
  （正拍照片可跳过锚点）

  想快速试一下？`python tools/make_sample_notebook.py out.png skew` 会生成一张样例作业本照片
  （`skew` = 斜拍梯形，`flat` = 正拍对照），并打印配套的锚点与关键曲线，可直接粘进预设文件。
...
### v2.9.0
详见 [`CHANGELOG.md`](CHANGELOG.md) 的 2.8.1 / 2.8.2 / 2.9.0 小节；设计文档见
...
- **新增「横线导引」**：沿作业本照片的印刷横线手绘曲线，文字逐行排布在横线内并跟随纸张弯曲。
  ...
  （曲线已含透视与弯曲的全部形变）
- **水平透视补偿**：斜拍时用锚点四角算出每行文字块的实际左边界与宽度，逐行平移 + 缩放，
  窄的一侧字更窄。锚点管页面几何、横线管每行弯曲，两者协同
- 新增 `ImageCanvasDialog` 基类，`CalibrationDialog` 与 `LineGuideDialog` 共用画布与坐标换算
- **修复**：CLI 加载预设时背景锚点校准永远不生效（`enabled = isValid()` 顺序反了）
